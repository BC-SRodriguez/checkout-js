...
const BlueSnapDirectEcpPaymentMethod: FunctionComponent<PaymentMethodProps> = ({
    ...
    const initializeEcp = useCallback(async () => {
        setValidationSchema(method, getEcpValidationSchema());

        await initializePayment({
            gatewayId: method.gateway,
            methodId: method.id,
        });
    }, [initializePayment, method, setValidationSchema]);
    ...
    return <BlueSnapDirectEcpFieldset onPermissionChange={onChange} />;
};
